...
if [[ -n "${SYNERGIUM_FORMS_PB_ADMIN_EMAIL:-}" && -n "${SYNERGIUM_FORMS_PB_ADMIN_PASSWORD:-}" ]]; then
  ...
  python3 "$REPO_ROOT/ops/scripts/synergium_forms_publish.py" rename --from "$FROM_ID" --to "$TO_ID"
fi
...
"${RSYNC[@]}" "$REPO_ROOT/producto/forms/deploy/pb_public/" "${USER}@${HOST}:${REMOTE_ROOT}/pb_public/"
"${RSYNC[@]}" "$REPO_ROOT/producto/forms/deploy/pb_hooks/" "${USER}@${HOST}:${REMOTE_ROOT}/pb_hooks/"
"${RSYNC[@]}" "$REPO_ROOT/producto/forms/deploy/scripts/setup_collections.py" "${USER}@${HOST}:${REMOTE_ROOT}/scripts/setup_collections.py"
"${RSYNC[@]}" "$REPO_ROOT/ops/scripts/synergium_forms_publish.py" "${USER}@${HOST}:${REMOTE_ROOT}/scripts/synergium_forms_publish.py"
