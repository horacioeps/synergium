...
Script: **`ops/scripts/vault_ssh_ro.sh`** (solo lectura).
...
# Leer nota
ops/scripts/vault_ssh_ro.sh cat "Ideas de negocio para ayudar investigadores - Synergium - y otras.md"

# Copiar a local para procesar
ops/scripts/vault_ssh_ro.sh get "ruta/nota.md" /tmp/nota.md

# Ruta remota canónica
ops/scripts/vault_ssh_ro.sh root
```
...
Usar `ops/scripts/vault_ssh_ro.sh find` con criterios acotados si no conoces la ruta exacta.

## Política

1. **Leer sí** cuando el pedido lo requiera.
2. **No escribir** en vault salvo petición explícita del usuario.
3. **No subir** contenido de vault a git salvo extractos derivados en `comercial/estrategia/` u otras carpetas de este repo (con criterio del usuario).
