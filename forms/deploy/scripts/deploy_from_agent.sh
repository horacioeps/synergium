...
# When called from /workspace/scripts → parent is workspace
if [[ -d /workspace/generado/synergium-forms/deploy ]]; then
  DEPLOY_SRC=/workspace/generado/synergium-forms/deploy
else
  DEPLOY_SRC="$(cd "$(dirname "$0")/.." && pwd)/deploy"
fi
...
"${RSYNC[@]}" \
  /workspace/scripts/synergium_forms_publish.py \
  "${USER}@${HOST}:${REMOTE_ROOT}/scripts/synergium_forms_publish.py"
"${SSH[@]}" "${USER}@${HOST}" "mkdir -p '${REMOTE_ROOT}/schemas'"
"${RSYNC[@]}" \
  /workspace/generado/synergium-forms/casos/community-directory-matching/schema-en.json \
