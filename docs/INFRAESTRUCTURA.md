...
- **Gestión:** REST API desde agente Cursor (scripts en `web/casos/*/apply.py`)
- **Patrón:** backup JSON antes de escribir → apply → rollback si falla
- **Idiomas:** ES + EN (TranslatePress u equivalente)

Casos documentados en [web/casos/](../web/casos/).
...
| Script | Uso |
|--------|-----|
| `scripts/synergium_forms_publish.py` | Publicar/cerrar/exportar forms |
| `forms/deploy/scripts/deploy_from_agent.sh` | Deploy desde agente |
| `web/casos/*/apply.py` | Cambios WP synergium.net |
