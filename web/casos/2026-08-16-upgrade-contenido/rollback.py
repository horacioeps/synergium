#!/usr/bin/env python3
"""Restaura synergium.net al estado previo al upgrade de contenido.
...
y los JSON en generado/web-synergium/wp-backups/pre-upgrade-2026-08-16/
...
BACK = pathlib.Path(__file__).resolve().parents[2] / "wp-backups" / "pre-upgrade-2026-08-16"
...
def main() -> None:
    ...
    header = load("header.json")
    nav = load("nav-27.json")
    ...
    jobs = [
        ...
        (f"{WP}/template-parts/twentytwentyfive//header", {
            "slug": "header",
        ...
        (f"{WP}/navigation/27", {
            "content": nav["content"]["raw"],
        }),
        (f"{WP}/settings", {
            "title": settings.get("title", "Synergium"),
            "description": settings.get("description", ""),
            "url": settings.get("url", "http://synergium.net"),
        }),
    ]
    ...
    # Deactivate plugins added by the upgrade if they are active.
    for slug in ("gtranslate/gtranslate", "synergium-seo/synergium-seo"):
        url = f"{WP}/plugins/{slug}"
        print(("DRY " if DRY else "DEACTIVATE ") + url)
        if DRY:
            continue
        st, body = req("POST", url, {"status": "inactive"})
        print(" ", st, body.get("status") if isinstance(body, dict) else str(body)[:160])
